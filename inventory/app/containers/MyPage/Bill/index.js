...
export function mapDispatchToProps(dispatch) {
  return {
    ...
    onPress__list_item : (item, sectionID, rowID) => {
      ...
      let path = {
        pathname:`/${PATH.PATH__bill__detail}`,
        state:item,
      };
      dispatch(push(path));
      // dispatch(push(`/${PATH.PATH__bill__detail}`));
...
const withReducer = injectReducer({ key : PATH.PATH__bill, reducer });
const withSaga = injectSaga({ key : PATH.PATH__bill, saga });
