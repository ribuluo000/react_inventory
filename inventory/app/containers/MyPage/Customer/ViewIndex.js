...
export default class ViewIndex extends BaseComponent {
  ...
  onPress__list_item = (item, sectionID, rowID)=>{
    ...
    if(pathname === `/${PATH.PATH__customer__select}`){
      onPress__list_item_select && onPress__list_item_select(item, sectionID, rowID)
    }else if(pathname === `/${PATH.PATH__customer}`){
  ...
  renderRow = (item, sectionID, rowID) => {
    ...
    const onPress__list_item = this.onPress__list_item;
  ...
  render() {
    ...
    return (
      <View>
        <Helmet>
          <title>{intl.formatMessage(messages.customer)}</title>
        </Helmet>
        <NavBar
          mode="dark"
          icon={<Icon type="left"/>}
