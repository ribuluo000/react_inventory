...
import { change_selected_batch } from "../../Bill/BillAdd/AddProduct/actions";
...
export function mapDispatchToProps(dispatch) {
  return {
    ...
    onPress__list_item : (item, sectionID, rowID) => {
      ...
      let path = {
        pathname:`/${PATH.PATH__product__batch__detail}`,
        state:item,
      };
      dispatch(push(path));
    ...
    onPress__list_item_select : (item, sectionID, rowID) => {
      ...
      dispatch(change_selected_batch(item));
      dispatch(goBack());
...
const withReducer = injectReducer({ key : PATH.PATH__product__batch, reducer });
const withSaga = injectSaga({ key : PATH.PATH__product__batch, saga });
